...
function unwrapKey(grantValue: string, storageKey: string) {
  const grant = JSON.parse(Buffer.from(decodeBase64Url(grantValue)).toString('utf8')) as {
    algorithm: string;
  ...
  const context = [
    'IMGX-GRANT-WRAP-v1', grant.version, grant.algorithm, grant.imageId,
    grant.issuedAt, grant.expiresAt, grant.nonce, grant.keyNonce,
    grant.signature, storageKey.trim().replace(/^\/+/, ''),
  ].join('.');
...
export async function openChapter(
  ...
  if (payload.length <= 33 || String.fromCharCode(...payload.slice(0, 4)) !== 'NOVL' || payload[4] !== 1) {
  ...
  decipher.setAAD(Buffer.from(`BFANG-NOVEL-v1.${config.storageKey.trim().replace(/^\/+/, '')}`));
...
export async function decryptImage(
  ...
  const payload = new Uint8Array(await response.arrayBuffer());
  if (payload.length <= 41 || String.fromCharCode(...payload.slice(0, 4)) !== 'IMGX' || payload[4] !== 3) {
  ...
  const grant = JSON.parse(Buffer.from(decodeBase64Url(asset.grant)).toString('utf8')) as { imageId: string };
  ...
  decipher.setAAD(Buffer.from(`IMGX-v3.${grant.imageId}.${asset.storageKey.trim().replace(/^\/+/, '')}.${width}.${height}`));
